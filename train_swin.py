...
Supports: swin_t, swin_s, swin_b, swin_v2_t, swin_v2_s, swin_v2_b
Usage: python train_swin.py --model swin_t --dataset pld
"""
import os
import sys
import random
...
from train_resnet18 import train_model, evaluate_on_test
...
def main():
    ...
    parser.add_argument('--lr', type=float, default=1e-4)  # Transformers usually use a smaller learning rate
    parser.add_argument('--num_workers', type=int, default=4)
    args = parser.parse_args()

    dataset_root = 'xx/Datasetforpt/dataset_origin'
    dataset_paths = {
    ...
    # Create model
    model = SWIN_MODELS[args.model](pretrained=True)
    # The classifier head for Swin Transformer is model.head
    in_features = model.head.in_features
    model.head = nn.Linear(in_features, num_classes)
    model = model.to(device)

    criterion = nn.CrossEntropyLoss()
    optimizer = optim.AdamW(model.parameters(), lr=args.lr, weight_decay=0.05)  # Swin recommends weight_decay=0.05
    ...
    ckpt_path = os.path.join(ckpt_dir, f'{args.model}_{args.dataset}_best.pth')
