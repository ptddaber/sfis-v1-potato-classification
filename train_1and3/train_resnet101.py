...
from trainer import train_model, evaluate_on_test  # noqa: E402

def main():
    root = 'xx/Datasetforpt/train_1and3/dataset'
    ...
    ckpt_dir = 'xx/Datasetforpt/train_1and3/checkpoints'
    os.makedirs(ckpt_dir, exist_ok=True)
    ckpt_path = os.path.join(ckpt_dir, 'resnet101_best.pth')
    torch.save({'model_state_dict': model.state_dict(), 'class_names': class_names}, ckpt_path)
    print(f'Saved best model to {ckpt_path}')

    evaluate_on_test(model, dataloaders, device, model_name="ResNet101")
